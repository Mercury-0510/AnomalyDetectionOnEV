...
class EVDataTransformer:
    ...
    def calculate_average_diff(self, df, columns):
        """
        计算指定列在每个时刻的平均变化量（当前时刻减去前一时刻）
        ...
        # 计算每列的差分（当前时刻减去前一时刻）
        diff_df = subset_df.diff()
        
        # 计算每行的平均变化量
        avg_diff = diff_df.mean(axis=1, skipna=True)  # axis=1表示按行计算平均值
        
        # 处理NaN值（第一行会是NaN），用0填充
        avg_diff = avg_diff.fillna(0.0)
        
        return avg_diff

    def add_calculated_channels(self, df):
        """
        为DataFrame添加计算的U_SD、T_SD、U_diff和T_diff列
        ...
        # 计算U_diff (电压平均变化量)
        df_copy['U_diff'] = self.calculate_average_diff(df_copy, u_columns)
        
        # 计算T_diff (温度平均变化量)
        df_copy['T_diff'] = self.calculate_average_diff(df_copy, t_columns)
    ...
    def _plot_file_sample_predictions(self, file_results, output_path):
        ...
        # 计算最大样本数量以自动调整图片宽度
        max_samples = max(len(file_results[f]['predictions']) for f in files_to_show)
        
        # 自动调整图片尺寸
        # 宽度根据样本数量调整，最小8，最大20
        fig_width = max(8, min(20, max_samples * 0.15 + 4))
        # 高度根据文件数量调整，每个文件0.3高度，最小2，最大10
        fig_height = max(2, min(10, len(files_to_show) * 0.3 + 1.5))
        
        fig, ax = plt.subplots(figsize=(fig_width, fig_height))
        
        # 设置颜色
        normal_color = "#0C6E0F"  # 绿色
        abnormal_color = "#AB2218"  # 红色
        ...
        for i, file_name in enumerate(files_to_show):
            ...
            for j, pred in enumerate(predictions):
                ...
                rect = patches.Rectangle((j, y_pos), 1, 0.6,  # 高度从0.6改为0.25
                                       linewidth=0.3, edgecolor='white', 
                                       facecolor=color)
                ax.add_patch(rect)
            
            # 添加文件名标签，去掉.csv后缀
            display_name = file_name.replace('.csv', '')  # 去掉.csv后缀
            display_name = display_name[:30] + '...' if len(display_name) > 30 else display_name
            ax.text(-0.5, y_pos + 0.125, display_name,  # 调整y位置以适应更矮的格子
        ...
        ax.legend(handles=[normal_patch, abnormal_patch], loc='center left', bbox_to_anchor=(1, 0.5))
    ...
    def transform_train_test(self, data_folder, output_dir='all_datasets/EV', train_ratio=0.5):
        ...
        for file in tqdm(train_files, desc="Processing testing files"):
        ...
        for file in tqdm(test_files, desc="Processing testing files"):
            csv_path = os.path.join(data_folder, file)
            df = pd.read_csv(csv_path)
            df = self.add_calculated_channels(df)
